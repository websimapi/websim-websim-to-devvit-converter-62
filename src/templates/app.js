...
router.post('/api/payments/fulfill', async (req, res) => {
    ...
    try {
        ...
        // In Devvit Web, the body often contains the order directly or wrapped.
        ...
        if (order.status === 'PAID') {
            ...
            if (amount > 0 && userId && postId) {
                ...
                try {
                    ...
                    console.warn('[Payment] Failed to post tip comment:', err);
                }
            } else {
                console.warn('[Payment] Missing context or invalid amount', { amount, userId, postId });
            }
        }
        
        // IMPORTANT: Must return success:true to acknowledge
        res.json({ success: true });
    } catch (e) {
        console.error("[Payment] Fulfillment Error:", e);
        // Return 200 with error info to prevent client retry loops if it's a logic error, 
